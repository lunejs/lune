...
export function createLocationCountryLoader(trx: Transaction) {
  return new DataLoader<string, Country>(async countriesIds => {
    const serializer = new CountrySerializer();
    const rows = await trx<CountryTable>(Tables.Country).whereIn('id', countriesIds).select('*');

    const map = new Map(rows.map(r => [r.id, serializer.deserialize(r) as Country]));
    return countriesIds.map(id => map.get(id) as Country);
  });
}
