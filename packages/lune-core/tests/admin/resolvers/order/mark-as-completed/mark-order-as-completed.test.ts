...
describe('markOrderAsCompleted - Mutation', () => {
  ...
  test('marks a delivered order as completed', async () => {
    const res = await request(app)
      .post('/admin-api')
      .set('Authorization', `Bearer ${UserConstants.AccessToken}`)
      .set('x_lune_shop_id', ShopConstants.ID)
      .send({
        query: MARK_ORDER_AS_COMPLETED_MUTATION,
        variables: {
          id: OrderConstants.DeliveredID
        }
      });

    const { order, apiErrors } = res.body.data.markOrderAsCompleted;
...
const MARK_ORDER_AS_COMPLETED_MUTATION = /* GraphQL */ `
  mutation MarkOrderAsCompleted($id: ID!) {
    markOrderAsCompleted(id: $id) {
      order {
        id
        state
        completedAt
