...
export const FulfillmentLineConstants = {
  ...
  AlreadyReadyFulfillmentLineID: TestUtils.generateUUID()
};
...
export class FulfillmentLineFixtures implements Fixture<FulfillmentLineTable> {
  ...
  async build(): Promise<Partial<FulfillmentLineTable>[]> {
    return [
      ...
      {
        shop_id: ShopConstants.ID,
        id: FulfillmentLineConstants.AlreadyReadyFulfillmentLineID,
        fulfillment_id: FulfillmentConstants.AlreadyReadyFulfillmentID,
        order_line_id: OrderLineConstants.AlreadyReadyOrderLineID,
        quantity: 1
      }
    ];
  }
