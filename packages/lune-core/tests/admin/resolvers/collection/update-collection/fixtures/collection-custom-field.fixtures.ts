...
export const CollectionCustomFieldConstants = {
  ExistingBannerTextFieldID: TestUtils.generateUUID(),
  ExistingBannerTextFieldValue: 'Winter Collection'
};
...
export class CollectionCustomFieldFixtures implements Fixture<CollectionCustomFieldTable> {
  ...
  async build(): Promise<Partial<CollectionCustomFieldTable>[]> {
    return [
      {
        id: CollectionCustomFieldConstants.ExistingBannerTextFieldID,
        value: JSON.stringify(CollectionCustomFieldConstants.ExistingBannerTextFieldValue),
