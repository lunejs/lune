...
import { MARK_ORDER_AS_COMPLETED_MUTATION } from '@/lib/api/operations/order.operations';
...
export const useMarkOrderAsCompleted = () => {
  ...
  const { isPending, mutateAsync } = useGqlMutation(MARK_ORDER_AS_COMPLETED_MUTATION);

  const markAsCompleted = async (orderId: string): Promise<ActionResult<OrderErrorCode>> => {
    try {
