...
import { MARK_FULFILLMENT_AS_DELIVERED_MUTATION } from '@/lib/api/operations/order.operations';
import type { OrderErrorCode } from '@/lib/api/types';
import type { ActionResult } from '@/shared/utils/result.utils';

import { OrderCacheKeys } from '../constants/cache-keys';

export const useMarkFulfillmentAsDelivered = () => {
  const queryClient = useQueryClient();
  const { isPending, mutateAsync } = useGqlMutation(MARK_FULFILLMENT_AS_DELIVERED_MUTATION);
