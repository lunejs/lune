    ...
    firstOption.values
      .flatMap(value => {
          ...
          return [
            {
              ...
              weight: variantAlreadyExists
                ? variantAlreadyExists.weight
                : (formValues?.weight ?? 0),
              length: variantAlreadyExists
                ? variantAlreadyExists.length
                : (formValues?.length ?? 0),
              height: variantAlreadyExists
                ? variantAlreadyExists.height
                : (formValues?.height ?? 0),
              width: variantAlreadyExists ? variantAlreadyExists.width : (formValues?.width ?? 0),
        ...
        return variants.map(variant => {
          ...
          const baseExists = existingVariants.find(
            variant => variant.values.map(v => v.name).join() === base
          );
          ...
          const variantToCreate = {
            ...
            weight: persistedVariant?.weight ?? formValues?.weight ?? 0,
            length: persistedVariant?.length ?? formValues?.length ?? 0,
            height: persistedVariant?.height ?? formValues?.height ?? 0,
            width: persistedVariant?.width ?? formValues?.width ?? 0,
