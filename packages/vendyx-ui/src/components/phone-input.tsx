...
export const PhoneInput = forwardRef<
  Component<
    ...
    unknown
  > | null,
  PhoneInputProps & { id?: string }
>(({ onChange, value, placeholder, className, id }, ref) => {
  const inputId = useId();
...
const CountrySelect = ({ disabled, value, onChange, options }: CountrySelectProps) => {
  ...
  return (
    <div className="relative inline-flex items-center self-stretch rounded-s-lg border border-input bg-accent py-2 pe-2 ps-3 text-muted-foreground transition-shadow focus-within:z-10 focus-within:border-ring focus-within:outline-none focus-within:ring-[3px] focus-within:ring-ring/20 hover:text-foreground has-[:disabled]:pointer-events-none has-[:disabled]:opacity-50">
